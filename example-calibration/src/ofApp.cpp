...
void ofApp::setup(){
    ...
    screenRect.set(0,0,1440,900);
    projectorRect.set(100,0,1280,720);
...
void ofApp::setupDefaultParams(){
    ...
    imageProcessingParams.add( circleDetectionThreshold.set("Circle image threshold", 250, 250, 250) );
}
...
void ofApp::setupGui(){
    ...
    setWindowProperty( "Pattern Window", WND_PROP_FULLSCREEN, CV_WINDOW_FULLSCREEN );
...
void ofApp::processImageForCircleDetection(cv::Mat img){
    ...
    Scalar hsv_l(110,50,50);//110,50,50
    Scalar hsv_h(130,255,255);//130,255,255
    Mat bw;
    ...
    //imshow("Projected Pattern",processedImg);
    //cv::waitKey(5);
...
void ofApp::drawProjectorPattern(){
    ...
    */
    ...
    Mat img = Mat::zeros(projectorRect.height, projectorRect.width, CV_8UC3);
    for(const auto & p : camProjCalib.getCalibrationProjector().getCandidateImagePoints()) {
        //ofCircle(p.x, p.y, 4);
        circle(img, p, 20, Scalar( 255, 0, 0), CV_FILLED); //blue circles
    }
    imshow("Pattern Window",img);
}
